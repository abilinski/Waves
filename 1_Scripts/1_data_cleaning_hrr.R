#### SETUP ####
here::i_am("1_Scripts/1_data_cleaning_hrr.R")
...
zip <- read.csv(here("0_Data", "Raw","ZIP_COUNTY_032023.csv"))%>%
  ...
  rbind(c(99658, 2158)) %>%   #3 counties missing from HUDs ZIP file
  rbind(c(99566, 2261)) %>%
  rbind(c(57770, 46102)) %>%
...
#get hrr population
c <- left_join(c,crosswalk, c('county_fips'='COUNTY'))
hrr_population <- aggregate(c,county_population ~ hrrnum,FUN = sum) %>%
  rename('hrr_population'='county_population')
c <- left_join(c,hrr_population, c('hrrnum'))
...
h = read.csv(here("0_Data", "Raw", "hosps_county.csv")) %>% 
  ...
  # group by HRR
  group_by(hrrnum, hrr_population, date) %>%
  summarize(
         n = n(),
  ...
  mutate(# admits
         ...
         admits_confirmed_100K = admits_confirmed_avg/hrr_population*100000,
         icu_100K = icu_confirmed_avg/hrr_population*100000/7) %>%
...
k = table(paste(h$hrrnum, h$date))
k[k > 1]

# check for completeness
chk_complete = h %>% filter(date >= "2021-02-01" & date <= "2022-10-01") %>%
  group_by(hrrnum) %>% summarize(n = n())
...
# View(df0 %>% filter(is.na(hrr_population)) %>% ungroup() %>% dplyr::select(hrrcity, hrrnum,fips) %>% unique())
...
df = df0 %>%
  ...
  mutate(cases_avg_per_100k = cases_avg/hrr_population*100000,
         ...
         POPESTIMATE2019 = hrr_population) %>%
  ...
  chk = paste(ymd, hrrnum)) %>% filter(ymd>=date_filter_start) %>% ungroup() %>%
...
## CHECKS ##
# check on lags
mean(d_out_pre_hrr$chk2 == d_out_pre_hrr$deaths_21_lag_100k, na.rm = T)

# check on missing data
d_out_pre_hrr %>% gather(var, value, deaths_weekly, admits_weekly, cases_weekly, perc_covid_100) %>%
  filter(date<="2022-10-01") %>%
  group_by(var) %>% summarize(sum(is.na(value)))

# check on missing data
d_out_pre_hrr %>% filter(is.na(hrr_population)) %>% ungroup() %>% dplyr::select(ymd, state, hrrcity, hrrnum) %>% unique()
# View(d_out_pre_hrr %>% group_by(ymd) %>% summarize(sum(hrr_population)))
